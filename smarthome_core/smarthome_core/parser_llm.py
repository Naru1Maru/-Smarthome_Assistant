...
LLM_UNIVERSAL_MAX_OUTPUT_TOKENS = 220
...
def _build_universal_prompt_payload(base_payload: Dict[str, Any]) -> Dict[str, Any]:
    """Add model-neutral guidance for models that were not tuned with baseline prompt."""
    payload = dict(base_payload)
    payload["universal_decision_order"] = [
        "1. Detect the user goal first: off, on, brightness, color, color temperature, or mood/scene.",
        "2. Use explicit words from utterance first, then context, then knowledge hints.",
        "3. If the utterance has a room, copy it exactly as canonical area_name from areas.",
        "4. If the utterance has no room, use context.selected_area_name, then context.last_area_name.",
        "5. Do not ask for clarification when context already provides the missing room.",
        "6. For creative everyday phrases, choose the closest safe one-step light action.",
        "7. Do not create schedules, automations, explanations, or multiple unrelated actions.",
        "8. Use UNKNOWN only when no safe lighting intent can be inferred.",
    ]
    payload["field_rules"] = {
        "target.scope": "AREA when area_name is known; ALL_LIGHTS only for explicit all/everywhere wording; UNSPECIFIED only when no target can be inferred.",
        "TURN_ON": "Use for turning light on and for combined absolute light settings with brightness/color/temp.",
        "TURN_OFF": "Use only for explicit off/dim-to-zero requests.",
        "SET_BRIGHTNESS": "Use only when brightness is the main absolute goal and no on/color/temp change is needed.",
        "ADJUST_BRIGHTNESS": "Use for relative brighter/dimmer requests.",
        "SET_COLOR": "Use for absolute RGB color. Set color_name and color_rgb together when known.",
        "SET_COLOR_TEMP": "Use for absolute warm/cold/neutral white temperature.",
        "ADJUST_COLOR_TEMP": "Use for relative warmer/cooler requests.",
        "MOOD_SCENE": "Use only when knowledge.scenes provides a matching scene preset.",
    }
    payload["few_shot_examples"] = [
        {
            "utterance": "выключи свет",
            "context": {"selected_area_name": "Спальня"},
            "output": {
                "schema_version": "1.0",
                "goal_type": "TURN_OFF",
                ...
                "scene_id": None,
                "clarification": {"needed": False, "question": "not needed", "options": []},
            },
        },
        {
            "utterance": "сделай свет как перед сном, не ярко",
            "context": {"selected_area_name": "Спальня"},
            "output_hint": "Use MOOD_SCENE when matching scene exists; otherwise TURN_ON with warm color temperature and low brightness.",
        },
    ]
    return payload


def _build_system_prompt(prompt_profile: str) -> str:
    profile = (prompt_profile or "baseline").strip().lower()
    if profile == "universal":
        return (
            "Ты строгий NLU-модуль для умного дома. "
            "Преобразуй русскую пользовательскую команду в LightingGoal v1 JSON. "
            "Верни ровно один JSON-объект без markdown, пояснений и текста вокруг. "
            "Используй только enum-значения и поля из JSON schema. "
            "Опирайся на utterance, context, areas, goal_contract, knowledge, universal_decision_order и field_rules. "
            "Не переводи названия комнат и не придумывай новые комнаты. "
            "Если комната не названа, используй context.selected_area_name, затем context.last_area_name. "
            "Для естественных творческих фраз выбирай ближайшее безопасное одноразовое действие со светом. "
            "Если knowledge.scenes.primary_scene_id существует для атмосферной фразы, scene_id не должен быть null. "
            "UNKNOWN используй только если нельзя безопасно вывести действие."
        )
    return (
...
class LLMParserV1:
    ...
    def parse(
        ...
        *,
        ...
        prompt_profile = (self.prompt_profile or "baseline").strip().lower()
        system = _build_system_prompt(prompt_profile)
        ...
        if prompt_profile == "universal":
            prompt_payload = _build_universal_prompt_payload(prompt_payload)
        ...
        max_tokens = LLM_UNIVERSAL_MAX_OUTPUT_TOKENS if prompt_profile == "universal" else LLM_MAX_OUTPUT_TOKENS
